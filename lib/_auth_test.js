...
// Deliberate (2026-07-25): act-as does NOT extend to chats. Threads are private even from
// admins — impersonation must never become a way to read someone's Coach conversations.
check('admin acting as X still chats as THEMSELVES (act-as ignored for chats)',
  auth.conversationUser(req({ mastery_user: auth.makeUserToken('info@agoradatadriven.com'), mastery_as: 'someone@x.com' })) === 'info@agoradatadriven.com');
check('Sentinel-role admin acting as X also chats as themselves',
  auth.conversationUser(req({ mastery_user: auth.makeUserToken('lance@agora.ph'), mastery_as: 'ianfernandezctm@gmail.com' })) === 'lance@agora.ph');
