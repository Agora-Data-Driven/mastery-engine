...
// --- buildFlowEdges: the spine follows the STORED order, not the names ---------
// Three grains, each numbered against the alphabet so a name-based sort would give
// the exact reverse. This is the regression the RAG track shipped with: lesson and
// course position used to be inferred from min(topic.order), so every 0-based
// per-lesson topic sequence collapsed them to a tie and the tree read alphabetically.
const row = (course, courseOrder, lesson, lessonOrder, topic, order) => ({
  id: `${course}/${lesson}/${topic}`, track: 'T', course, lesson, topic, courseOrder, lessonOrder, order,
});
const flow = buildFlowEdges([
  // Course "Zebra" is FIRST (courseOrder 0) though it sorts last by name.
  row('Zebra', 0, 'Yak', 0, 'Xi', 0),
  row('Zebra', 0, 'Yak', 0, 'Wu', 1),
  row('Zebra', 0, 'Vole', 1, 'Umi', 0),   // lesson "Vole" is SECOND though it sorts first
  row('Apple', 1, 'Bee', 0, 'Cat', 0),
]);
const chain = ['Zebra/Yak/Xi', 'Zebra/Yak/Wu', 'Zebra/Vole/Umi', 'Apple/Bee/Cat'];
check('flow spine follows courseOrder / lessonOrder / order, never the names',
  same(flow, chain.slice(1).map((to, i) => ({ from: chain[i], to, kind: 'flow' }))));

// Per-lesson 0-based topic order (what every AI sequencing pass writes) must NOT be
// read as a lesson's position: both lessons below start at 0 and lessonOrder decides.
const flow2 = buildFlowEdges([
  row('C', 0, 'Second', 1, 'b1', 0),
  row('C', 0, 'First', 0, 'a1', 0),
]);
check('0-based topic order in every lesson does not flatten lesson order',
  same(flow2, [{ from: 'C/First/a1', to: 'C/Second/b1', kind: 'flow' }]));

// With no stored ranks at all, the old name-based behaviour still holds.
const flow3 = buildFlowEdges([
  { id: 'n2', track: 'T', course: 'C', lesson: 'B', topic: 't' },
  { id: 'n1', track: 'T', course: 'C', lesson: 'A', topic: 't' },
]);
check('unranked curriculum still falls back to natural name',
  same(flow3, [{ from: 'n1', to: 'n2', kind: 'flow' }]));
