...
export async function callAnthropic(prompt, { json = false, model, maxTokens, meta } = {}) {
  ...
  // "max_tokens" here means the answer was cut off at the ceiling — the difference between a model
  // that wrote bad JSON and one that never got to finish writing good JSON (gemini.js fillEngineMeta).
  if (meta) meta.finishReason = data?.stop_reason || '';
