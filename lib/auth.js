 ...
 * (chats) NEVER default to the legacy owner — see conversationUser.
 */
 ...
 * request reads/writes. Unlike effectiveUser, an admin who isn't impersonating
 * keys chats to their OWN email — the legacy-owner default exists for progress
 * continuity, but chats have no legacy migration story and are private, so
 * admin A must never read (or write into) another account's threads.
 */
