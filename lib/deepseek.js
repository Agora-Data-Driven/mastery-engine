...
/**
 * DeepSeek V4's thinking toggle defaults to ENABLED server-side, so the "fast"
 * Flash model silently runs a full chain-of-thought unless we opt out. Mirroring
 * the Gemini path, only an explicit `thinking === false` disables it; otherwise
 * we omit the field and let the model's default stand.
 */
function thinkingField(thinking) {
  return thinking === false ? { thinking: { type: 'disabled' } } : {};
}

/** Single-prompt completion. Mirrors callGemini's contract (returns text, and
 *  reports why it stopped on `meta.finishReason` — see gemini.js fillEngineMeta). */
export async function callDeepSeek(prompt, { json = false, model, thinking, meta } = {}) {
  ...
  // "length" here means the answer was cut off at the token ceiling — the difference between a
  // model that wrote bad JSON and one that never got to finish writing good JSON.
  if (meta) meta.finishReason = data?.choices?.[0]?.finish_reason || '';
 ...
 * DeepSeek emits OpenAI-style `data: {choices:[{delta:{content, reasoning_content}}]}`
 * lines — the thinking models put their chain-of-thought in `reasoning_content`.
 * Callers that ignore the 2nd arg (the learner endpoints) just see content.
 */
export async function streamDeepSeek(prompt, model, onToken, { thinking } = {}) {
  if (!DEEPSEEK_KEY) throw new Error('DEEPSEEK_API_KEY is not configured');
  const res = await fetch(URL, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${DEEPSEEK_KEY}` },
