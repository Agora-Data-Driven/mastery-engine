...
/* The always-available floating assistant keeps ONE running thread per user
 * (context is passed per message, so no per-scope threads). Stored alongside the
 * scope chats under a fixed id. */
const ASSISTANT_ID = 'assistant';

export async function getAssistantChat(userKey) {
  return getScopeChat(userKey, ASSISTANT_ID);
}
export async function saveAssistantChat(userKey, messages) {
  return saveScopeChat(userKey, ASSISTANT_ID, (Array.isArray(messages) ? messages : []).slice(-40));
}
