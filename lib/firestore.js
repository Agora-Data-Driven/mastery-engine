 ...
 *                 items: [ { topicId, track, course, lesson, topic, note } ] } ],
 *     source, createdBy, createdAt, updatedBy, updatedAt }
 * The id defaults to slug(title) so re-planning the same goal UPDATES in place
 * rather than spawning duplicates; pass an explicit id to target one. Each item
 * carries BOTH the stable topic doc id AND the full name-path, so the learner UI
 * can join to catalog mastery by id and launch quizzes/reviews by name (the two
 * ways the rest of the app addresses a topic).
 */

/** Coerce one stage into the stored shape (ids stable within a roadmap, fields bounded). */
function normRoadmapStage(s, i) {
  ...
  return {
    ...
    items: items
      .map((it) => ({
        topicId: String(it?.topicId || slug(it?.track || '', it?.course || '', it?.lesson || '', it?.topic || '')),
...
export async function saveRoadmap(rm) {
  ...
  const doc = {
    ...
    program: String(rm?.program || DEFAULT_PROGRAM),
    audience: rm?.audience === 'everyone' ? 'everyone' : 'program',
    stages: (Array.isArray(rm?.stages) ? rm.stages : []).map(normRoadmapStage),
