 ...
 * `thinking` (Gemini only) turns extended thinking on/off; `schema` (Gemini only)
 * is a responseSchema that guarantees the JSON shape at decode time.
 */
...
Return ONLY a JSON object: {"question": "text", "options": ["A", "B", "C", "D"], "answerIndex": 0}`;
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    let obj;
    try {
      obj = JSON.parse(await complete(prompt, { json: true, schema: MCQ_SCHEMA, ...ai }));
    } catch {
...
Return ONLY a JSON array of ${n} objects: [{"question": "text", "options": ["A", "B", "C", "D"], "answerIndex": 0}]`;
  ...
  let generated;
  try {
    generated = JSON.parse(await complete(prompt, { json: true, schema: MCQ_ARRAY_SCHEMA, ...ai }));
  } catch {
