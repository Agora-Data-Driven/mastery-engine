...
import { callDeepSeek, streamDeepSeek, deepseekConfigured } from './deepseek.js';
import { callKimi, streamKimi, kimiConfigured } from './kimi.js';
import { callAnthropic, streamAnthropic, anthropicConfigured } from './anthropic.js';
...
`;
  ...
  if (removed.length) {
    block += `\nSECTIONS TEMPORARILY REMOVED (PARKED) FROM THEIR ENGINE — the learner set these aside for now. They are hidden from their Progress, quizzes and drills, but NOT deleted: the content stays in the bank and is restorable anytime. You CAN see and discuss these, say plainly that they're parked when asked, and propose restore_section (exact names from here) when the learner wants one back. NEVER count them in "what's in my engine" answers or include them in a suggested study path unless the learner asks to bring them back:
...
function holisticBlock(profile) {
  ...
  const arLines = Object.entries(ar).map(([dim, a]) =>
    `${dim}${a && a.deadline ? ` (deadline ${a.deadline})` : ''}${a && a.other_info ? ` — notes: ${a.other_info}` : ''}`);
  if (arLines.length) lines.push(`- Growth areas (editable via update_area): ${arLines.join(' | ')}.`);
...
function assistantActionBlock(profile, enabled, hostFrame = true) {
  if (!enabled) return '';
  const engineOps = `THEIR MASTERY ENGINE — you can propose REMOVING or RESTORING a section of it, at any grain (a whole track, a course, a lesson, or one sub-lesson). Use this when something is blocking or overwhelming them and you recommend parking it for now: removal is TEMPORARY and never destructive — the content stays in the shared bank and in every roadmap, and can be restored anytime. When you recommend a removal, say WHY, propose it, and reassure them you'll help bring it back later.
  remove_section {track, course?, lesson?, topic?}  — removes the DEEPEST level named (\`topic\` = one sub-lesson/card). Name the FULL path down to it (track, then course/lesson/topic as applicable), copying every name VERBATIM from THE LEARNER'S MASTERY ENGINE list above. Never guess or shorten a name.
  restore_section {track, course?, lesson?, topic?}  — brings a removed section back. Removed sections are listed under "SECTIONS TEMPORARILY REMOVED (PARKED)" above — copy the exact names from there (or from earlier in this conversation).
Example — if you agree to skip one sub-lesson for now:
\`\`\`agora-action
...
Available ops and their args:
  ...
  update_area {dimension(spiritual|professional|philosophical|physical), deadline?(YYYY-MM-DD, null = back to default), other_info?}  — a growth AREA's pace deadline and its free-form "Other info" notes on the Overview. other_info REPLACES the stored text, so when adding to it, resend the existing content (under "areas" below) plus the addition.
...
export async function generateAssistantChat({ context = {}, history = [], message, conversational = false, search = false, catalog = [], transcripts = [], coach = false, progress = '', holistic = null, actions = false, hostFrame = false, attachments = [], admin = false }, ai = {}) {
 ...
 */
export async function streamAssistantChat({ context = {}, history = [], message, steer = '', catalog = [], transcripts = [], search = false, coach = false, progress = '', holistic = null, actions = false, hostFrame = false, attachments = [], admin = false }, ai = {}, onToken) {
