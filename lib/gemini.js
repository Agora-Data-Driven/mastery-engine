...
async function completeStream(prompt, { provider, model, thinking, thoughts } = {}, onToken) {
  ...
  return streamGemini(prompt, onToken, model, thinking, thoughts);
}
...
async function streamGemini(prompt, onToken, model, thinking, thoughts) {
  const useModel = model || MODEL;
  ...
  if (thoughts && thinking !== false) gc.thinkingConfig = { ...(gc.thinkingConfig || {}), includeThoughts: true };
 ...
 */
function assistantCatalogBlock(catalog) {
  if (!Array.isArray(catalog) || !catalog.length) return '';
  const outline = catalogOutline(catalog, 32000);
  return `THE ACTUAL CURRICULUM available to this student — Track > Course > Lesson, then each lesson's sub-lessons. These sub-lessons ARE the "cards"/"topics" they can study; this list is the ONLY content that exists:
${outline}

HARD RULE — CONTENT LOCATION: when the student asks where something is taught, or which card / sub-lesson / lesson / course / track covers a concept, answer ONLY with names copied VERBATIM from the list above. NEVER invent, rename, or guess a card/topic/lesson/section that is not in the list. If nothing clearly matches, say so plainly (e.g. "I don't see a dedicated card on that exact thing") and point to the closest real items that do exist. Do not present an inferred or "should-exist" name as if it were a real card.`;
}
...
export async function generateAssistantChat({ context = {}, history = [], message, conversational = false, search = false, catalog = [] }, ai = {}) {
  ...
  const catalogGround = assistantCatalogBlock(catalog);
 ...
 */
export async function streamAssistantChat({ context = {}, history = [], message, steer = '', catalog = [] }, ai = {}, onToken) {
  const persona = `You are the always-available Study Assistant inside the AGORA Mastery Engine — a spaced-repetition mastery-learning app — helping a working data/AI engineer. You can SEE what's on the student's screen (below) and answer with that context in mind, like a tutor over their shoulder. Be direct, warm, and concrete.`;
  const catalogGround = assistantCatalogBlock(catalog);
...
Do NOT use em dashes; use commas, colons, or simple hyphens.${steerNote(steer)}`;
  const reply = await streamStructured(prompt, ai, onToken);
