...
function holisticBlock(profile) {
  ...
  if (gym.weekly_split) {
    const split = Object.entries(gym.weekly_split).map(([d, t]) => `${d} ${t}`).join(', ');
    lines.push(`- Gym plan (recurring weekly split): ${split}. Trained ${gym.sessions_last_14d ?? 0} time(s) in the last 14 days (${gym.completed_last_14d ?? 0} full sessions).`);
  }
...
function assistantActionBlock(profile, enabled) {
  ...
  const gymNow = gym.weekly_split
    ? `\nTheir current weekly gym split (edit with the gym ops below): ${Object.entries(gym.weekly_split).map(([d, t]) => `${d}=${t}`).join(', ')}.\n`
    : '';
...
GYM SCHEDULE — the recurring weekly split shown on their calendar (day-types: Push, Pull, Legs, Custom, Rest):
  set_gym_week {week}  — replace the WHOLE weekly split. \`week\` is a full map with keys Mon,Tue,Wed,Thu,Fri,Sat,Sun.
  set_gym_day {date, day_type}  — override ONE date (ISO YYYY-MM-DD), e.g. move a split onto it or mark it Rest.
...
\`\`\`
Example — if they say "make Wednesdays a rest day and swap Friday to legs":
\`\`\`agora-action
{"op":"set_gym_week","args":{"week":{"Mon":"Push","Tue":"Pull","Wed":"Rest","Thu":"Push","Fri":"Legs","Sat":"Pull","Sun":"Rest"}},"summary":"Set Wednesday to Rest and Friday to Legs"}
\`\`\``;
}
