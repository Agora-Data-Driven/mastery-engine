...
/**
 * Optional admin steer for a generation run — the admin's own words on what kind of
 * questions/cards they want (e.g. "more application scenarios", "focus on RNNs",
 * "harder edge cases"). Injected as high-priority guidance on emphasis/style, but it
 * never overrides accuracy or the formatting rules. Empty when no guidance is given.
 */
function guidanceBlock(instructions) {
  const s = String(instructions || '').trim().slice(0, 2000);
  if (!s) return '';
  return `\nADMIN GUIDANCE (what kind of questions to write — follow this closely for emphasis, angle and difficulty, but NEVER at the expense of accuracy, grounding, or the formatting rules):\n"""\n${s}\n"""\n`;
}
...
function catalogOutline(catalog) {
  const tree = new Map();
 ...
 */
export async function classifyTranscript({ transcript, catalog = [], programName = '' }, ai = {}) {
...
Return ONLY JSON: {"title":"...","summary":"...","track":"...","course":"...","lesson":"...","topics":["...","..."]}`;
  ...
  const text = await complete(prompt, { json: true, schema: PLACEMENT_SCHEMA, ...ai });
  let out;
...
Return ONLY JSON: {"track":"...","course":"...","summary":"...","assumedKnowledge":["..."],"lessons":[{"lesson":"...","rationale":"...","topics":["...","..."]}]}`;
  ...
  const text = await complete(prompt, { json: true, schema: PLAN_SCHEMA, ...ai });
  let out;
  try { out = parseLooseJson(text); } catch { throw new Error('The model returned non-JSON content'); }
...
export async function generateAssistantChat({ context = {}, history = [], message, conversational = false, search = false }, ai = {}) {
