...
/**
 * Blocking Gemini call with an arbitrary `parts` array (multimodal: mix of
 * { text } and { inlineData: { mimeType, data } }). Used to read uploaded
 * PDFs/images natively. `maxOutputTokens` lets a long transcription run to
 * completion. Thinking is left off (Flash) so transcription is fast and cheap.
 */
async function callGeminiParts(parts, { model, maxOutputTokens } = {}) {
  const useModel = model || MODEL;
  const { url, token } = await vertexTarget(useModel, 'generateContent');
  const body = { contents: [{ role: 'user', parts }] };
  const gc = geminiGenConfig(useModel, { thinking: false }) || {};
  if (maxOutputTokens) gc.maxOutputTokens = maxOutputTokens;
  ...
  const data = await res.json();
  // A multi-part answer (rare) is joined; a transcription may span several parts.
...
function buildPrompt(topic, baseline, count, { existing = [], performance = null, difficulty = 'auto', prereqs = [] } = {}) {
...
export async function generateQuestions(topic, baseline, count, ai = {}, ctx = {}) {
  const { existing = [], performance = null, difficulty = 'auto', prereqs = [] } = ctx;
  const text = await complete(buildPrompt(topic, baseline, count, { existing, performance, difficulty, prereqs }), { json: true, schema: MCQ_ARRAY_SCHEMA, ...ai });
...
/**
 * Faithfully transcribe ONE uploaded document (PDF or image) to plain text via
 * Gemini's native multimodal reading. Text-like files (txt/md/csv…) are decoded
 * directly by the caller and never reach here. Always routes through Gemini
 * (Vertex ADC) regardless of the quiz-generation provider, because it is the
 * only always-available engine that can read a PDF/image on the deployed app.
 * Returns plain text (math kept as LaTeX); the model is told to be a transcriber,
 * not a summariser, so nothing the learner uploaded is silently dropped.
 */
export async function extractDocumentText({ mimeType, dataBase64, name = '' } = {}) {
  if (!mimeType || !dataBase64) return '';
  const prompt = `You are a precise document transcriber. Read the attached file${name ? ` ("${name}")` : ''} and output its FULL textual content as clean plain text.

RULES:
- Transcribe ALL text faithfully, in reading order. Do NOT summarise, shorten, or add commentary.
- Preserve structure with simple markdown: use "# " / "## " for headings and "- " for bullet/list items. Keep tables as readable rows.
- Render every mathematical expression in LaTeX: inline as $...$ and display as $$...$$ (never bare Unicode math symbols).
- For a diagram, chart, or figure that carries meaning, add one line: "[Figure: <a brief factual description of what it shows>]".
- If a page is blank or unreadable, skip it silently. Output ONLY the transcribed content, no preamble.`;

  // A generous cap so long lecture PDFs aren't truncated mid-transcription.
  const parts = [{ text: prompt }, { inlineData: { mimeType, data: dataBase64 } }];
  const text = await callGeminiParts(parts, { model: MODEL, maxOutputTokens: 16384 });
  return String(text || '').trim();
}

/**
 * Generate a mastery quiz from arbitrary SOURCE MATERIAL the learner supplied
 * (pasted text and/or uploaded documents, already extracted to plain text) —
 * NOT from the topic catalog. Reuses the exact same question-craft as the
 * catalog generator: the "Wise Master Educator / Professional Test Developer"
 * framing, the anti-test-hacking formatting contract (option uniformity, no
 * length bias, parallel structure, sophisticated distractors), the difficulty
 * directive, KaTeX/answer-index rules, the responseSchema and normaliser.
 *
 * Difficulty is manual here (there is no per-topic history), so 'auto' maps to
 * BALANCED. `extraContext` is the learner's optional free-text steer (bounded by
 * the caller). Questions are grounded ONLY in the supplied material. For larger
 * counts it generates in sequential waves, feeding earlier stems back as an
 * avoid-list so a big quiz stays distinct and never truncates a giant JSON blob.
 *
 * @returns {Promise<Array<{topic,question,options,answer}>>}
 */
export async function generateQuestionsFromContent(
  { content, sourceLabel = 'Your material', extraContext = '', count = 5, difficulty = 'auto' },
  ai = {},
) {
  const material = String(content || '').trim();
  if (!material) throw new Error('No source material to build a quiz from');

  const total = Math.min(30, Math.max(1, parseInt(count, 10) || 5));
  const level = ['core', 'balanced', 'challenge'].includes(difficulty) ? difficulty : 'balanced';
  const steer = String(extraContext || '').trim().slice(0, 1200);

  const build = (n, avoid) => `You are a Wise Master Educator and Professional Test Developer. A learner has given you their OWN study material and wants a quiz that tests genuine mastery of it.
...
${material}
"""
${steer ? `\nADDITIONAL INSTRUCTIONS FROM THE LEARNER (honour these when they don't conflict with staying grounded in the material):\n"""\n${steer}\n"""\n` : ''}
${DIFFICULTY_DIRECTIVE[level]}

${avoidBlock(avoid, 'do NOT duplicate or paraphrase any of these')}
YOUR MISSION:
...
- No "Length Bias": Do not make the correct answer the longest or most detailed.
- PARALLEL STRUCTURE: If one option starts with a verb, all must start with a verb. Keep the phrasing symmetrical.
- SOPHISTICATED DISTRACTORS: Ensure wrong answers are plausible and reflect common misconceptions about this material.
...
Return ONLY a JSON array of ${n} objects: [{"question": "text", "options": ["A", "B", "C", "D"], "answerIndex": 0}]`;
  ...
  const out = [];
  const WAVE = 8; // keep each JSON payload small enough to never truncate
  while (out.length < total) {
    const n = Math.min(WAVE, total - out.length);
    const avoid = out.map((q) => q.question);
    let batch;
    try {
      batch = parseLooseJson(await complete(build(n, avoid), { json: true, schema: MCQ_ARRAY_SCHEMA, ...ai }));
    } catch {
      throw new Error('Quiz generator returned non-JSON content');
    }
    if (!Array.isArray(batch)) batch = [batch];
    const clean = batch.map((q) => normalizeMcq(q, sourceLabel)).filter(Boolean);
    // A wave that yields nothing usable means the material is exhausted (or the
    // model is stalling) — stop rather than loop forever on the same avoid-list.
    if (!clean.length) break;
    for (const q of clean) {
      if (out.length >= total) break;
      out.push(q);
    }
    if (clean.length < n) break; // model couldn't fill the wave; material likely exhausted
  }
