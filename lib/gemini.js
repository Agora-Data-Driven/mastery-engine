 ...
 * switched per request: { provider: 'ollama'|'gemini', model } (defaults Gemini).
 */
 ...
 */
export async function generateHint({ question, options, answer }, ai = {}) {
...
${LATEX_RULE}`;
  return (await complete(prompt, ai)).trim();
}

/**
 * A from-scratch explanation aimed at a complete beginner, given after answering.
 */
export async function generateExplanation({ question, options, answer, userAnswer, isCorrect }, ai = {}) {
 ...
 * answers.
 */
export async function generateReview({ scopeLabel, topics, questions }, ai = {}) {
 ...
 */
export async function generateAnalysis({ overall, byCourse, weakest }, ai = {}) {
