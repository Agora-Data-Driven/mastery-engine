 ...
 */
export async function generateFlashcardQuestion({ topic, scopeLabel, concept, intuition, formula }, ai = {}, { brief = '', siblings = [], mode = '' } = {}) {
...
export async function generateFlashcardQuestions(card, count, ai = {}, ctx = {}) {
  ...
  const { existing = [], performance = null } = ctx;
  ...
  const results = await mapWithConcurrency(briefs, 4, (brief, i) =>
    generateFlashcardQuestion(card, ai, { brief, mode, siblings: briefs.filter((_, j) => j !== i) })
      .catch(() => null));
...
/** Render learner performance for a generation prompt (null-safe). */
function performanceBlock(performance) {
  if (!performance || performance.accuracy == null) {
    return 'LEARNER PERFORMANCE: no attempts recorded yet — assume a first pass and cover the core of the concept.';
  }
  ...
  }\nAim difficulty accordingly: reinforce fundamentals when accuracy is low, push edge cases when it is high.`;
}
 ...
 */
export async function planQuestions({ label, context, existing = [], performance = null, count }, ai = {}) {
  const n = Math.min(10, Math.max(1, parseInt(count, 10) || 1));
...
function buildPrompt(topic, baseline, count, { existing = [], performance = null } = {}) {
...
export async function generateQuestions(topic, baseline, count, ai = {}, ctx = {}) {
  const { existing = [], performance = null } = ctx;
  const text = await complete(buildPrompt(topic, baseline, count, { existing, performance }), { json: true, schema: MCQ_ARRAY_SCHEMA, ...ai });
