 ...
 */
async function complete(prompt, { json = false, provider, model, thinking, schema, search = false } = {}) {
  // `search` (Google Search grounding / internet access) is a Gemini-via-Vertex capability only;
  // the other providers don't support it here, so it's silently ignored for them.
  ...
  return callGemini(prompt, { json, model, thinking, schema, search });
}
...
async function completeStream(prompt, { provider, model, thinking, thoughts, search } = {}, onToken) {
  ...
  // `search` (Google Search grounding) is a Gemini-via-Vertex capability only.
  return streamGemini(prompt, onToken, model, thinking, thoughts, !!search);
}
...
async function callGemini(prompt, { json = false, model, thinking, schema, search = false } = {}) {
...
async function streamGemini(prompt, onToken, model, thinking, thoughts, search = false) {
  const useModel = model || MODEL;
...
function catalogOutline(catalog, cap = 12000) {
  const tree = new Map();
  ...
  const lines = [];
  for (const [key, topics] of tree) {
    lines.push(key);
    for (const t of topics) lines.push(`    - ${t}`);
  }
  const text = lines.join('\n');
  // A mature program can be hundreds of lessons; keep the prompt bounded.
 ...
 */
function assistantCatalogBlock(catalog, transcripts = []) {
  if ((!Array.isArray(catalog) || !catalog.length) && (!Array.isArray(transcripts) || !transcripts.length)) return '';
  let block = '';
  if (Array.isArray(catalog) && catalog.length) {
    block += `THE ACTUAL CURRICULUM available to this student — Track > Course > Lesson, then each lesson's sub-lessons. These sub-lessons ARE the "cards"/"topics" they can study; this list is the ONLY content that exists:
${catalogOutline(catalog, 32000)}
`;
  }
 ...
 */
function assistantCoachBlock(coach, progress) {
  if (!coach) return '';
  const prog = String(progress || '').trim();
  return `${prog ? `THE STUDENT'S PROGRESS SO FAR (calibrate to this — do NOT re-teach what they've mastered; start where they're weak or haven't begun):\n${prog}\n\n` : ''}COACH MODE IS ON. Structure EVERY reply as:
1. A concise answer to their question.
2. Then, ALWAYS, a section headed exactly "**Suggested path to drill this**" followed by a NUMBERED list of 2 to 6 real sub-lessons drawn VERBATIM from the curriculum above — the concrete lessons they should practise, in order (prerequisites first), tailored to their progress: skip what they've mastered, start where they're weak or haven't begun, and end at whichever existing sub-lesson most directly covers (or best builds toward) what they asked about.
This "Suggested path to drill this" section is REQUIRED on every coach-mode reply — never omit it. If the exact concept has no dedicated sub-lesson, say so briefly, then still list the closest real sub-lessons that build toward it. Every item MUST be a sub-lesson that literally appears in the curriculum above — never invent one.`;
}
...
export async function generateAssistantChat({ context = {}, history = [], message, conversational = false, search = false, catalog = [], transcripts = [], coach = false, progress = '' }, ai = {}) {
  ...
  const catalogGround = assistantCatalogBlock(catalog, transcripts);
  const coachGround = assistantCoachBlock(coach, progress);

  const persona = `You are the always-available Study Assistant inside the AGORA Mastery Engine — a spaced-repetition mastery-learning app — helping a working data/AI engineer. You can SEE what's on the student's screen (below) and answer with that context in mind, like a tutor over their shoulder. Be direct, warm, and concrete.`;
...
${LATEX_RULE}
Do NOT use em dashes; use commas, colons, or simple hyphens.`;
    const text = await complete(prompt, { ...ai, search: true });
 ...
 */
export async function streamAssistantChat({ context = {}, history = [], message, steer = '', catalog = [], transcripts = [], search = false, coach = false, progress = '' }, ai = {}, onToken) {
...
Do NOT use em dashes; use commas, colons, or simple hyphens.${steerNote(steer)}`;
  const reply = await streamStructured(prompt, { ...ai, search: canWeb }, onToken);
