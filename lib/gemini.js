...
export async function generateFlashcardQuestions(card, count, ai = {}, ctx = {}) {
  ...
  const { existing = [], performance = null, difficulty = 'auto' } = ctx;
 ...
 */
export async function planQuestions({ label, context, existing = [], performance = null, difficulty = 'auto', count }, ai = {}) {
  const n = Math.min(10, Math.max(1, parseInt(count, 10) || 1));
...
function buildPrompt(topic, baseline, count, { existing = [], performance = null, difficulty = 'auto' } = {}) {
...
export async function generateQuestions(topic, baseline, count, ai = {}, ctx = {}) {
  const { existing = [], performance = null, difficulty = 'auto' } = ctx;
  const text = await complete(buildPrompt(topic, baseline, count, { existing, performance, difficulty }), { json: true, schema: MCQ_ARRAY_SCHEMA, ...ai });
