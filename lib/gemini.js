 ...
 * this artifact, rendered into the prompt. Empty on a first build. Shared by
 * generateReview and generateLesson, so 'regenerate with feedback' means the
 * same thing on both buttons (generateVisualGuide words its own, because it is
 * fixing a page rather than prose).
 */
 ...
 * `generateLesson`.
 */
export async function generateReview({ scopeLabel, topics, questions, critique = '' }, ai = {}, onToken) {
...
export async function generateLesson(
  { scopeLabel, topics, questions, prereqs = [], dependents = [], critique = '' },
  ai = {},
  onToken,
) {
...
export async function generateVisualGuide(
  { scopeLabel, kind = 'review', topics = [], guide = '', questions = [], critique = '' },
  ai = {},
  onToken,
) {
...
${topics.map((t) => `- ${t}`).join('\n')}
${critiqueBlock}
THE WRITTEN ${kind === 'lesson' ? 'LESSON' : 'REVIEW'} THE LEARNER IS READING — visualise THIS material, do not invent a different syllabus:
"""
...
1. Read the whole lesson first. Pick the 3 to 6 ideas that matter most for real understanding. Skip trivia.
...
3. Order them simple to complex. Visual 1 must make sense to a complete beginner; later ones may go deeper.
...
- ONE <style> and ONE <script>, both inline in the document. Plain vanilla JavaScript.
...
export async function generateAssistantChat({ context = {}, history = [], message, conversational = false, search = false, catalog = [], transcripts = [], coach = false, progress = '', holistic = null, mentorHits = null, growthHits = null, work = null, workHits = null, actions = false, hostFrame = false, attachments = [], admin = false }, ai = {}) {
 ...
 */
export async function streamAssistantChat({ context = {}, history = [], message, steer = '', catalog = [], transcripts = [], search = false, coach = false, progress = '', holistic = null, mentorHits = null, growthHits = null, work = null, workHits = null, actions = false, hostFrame = false, attachments = [], admin = false }, ai = {}, onToken) {
