...
async function callGemini(prompt, { json = false, model } = {}) {
  const { url, token } = await vertexTarget(model || MODEL, 'generateContent');
  const body = {
  ...
  recordGeminiUsage(model || MODEL, data.usageMetadata);
  return text;
}
...
async function streamGemini(prompt, onToken, model) {
  const { url, token } = await vertexTarget(model || MODEL, 'streamGenerateContent', true);
  const res = await fetch(url, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
  ...
  recordGeminiUsage(model || MODEL, usage);
}
...
Return ONLY a JSON array of ${n} objects: [{"topic": "${topic}", "question": "text", "options": ["A", "B", "C", "D"], "answer": "exact correct option text"}]`;

  let generated;
  try {
    generated = JSON.parse(await complete(prompt, { json: true, ...ai }));
  } catch {
