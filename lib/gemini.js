...
export async function generateAssistantChat({ context = {}, history = [], message, conversational = false }, ai = {}) {
  // Spoken mode: the reply is read aloud by a TTS voice, so markdown (headings, bullets, code
  // fences, tables) gets read out literally ("hashtag hashtag", "star star"). Ask for natural
  // talk instead — plain sentences, brief, no formatting, no visual.
