...
export async function stepGenJob(jobId, ai = {}) {
  ...
  try {
    await runWithUsage(usage, async () => {
      ...
      const gopts = { provider: job.provider || 'deepseek', ...(job.model ? { model: job.model } : {}), ...ai };
      ...
      const generated = job.grounding === 'topic'
        ? await generateAcademyQuestions(
          { topic, scopeLabel, reference: source, existing: existing.map((q) => q.question), count: want, instructions: job.instructions || '' },
          gopts,
        )
        : await generateQuestionsFromTranscript(
          { topic, scopeLabel, transcript: source, existing: existing.map((q) => q.question), count: want, instructions: job.instructions || '' },
          gopts,
        );
