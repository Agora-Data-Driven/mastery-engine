...
import {
  getGenJob, updateGenJob, getTranscripts, addQuestion, bumpQuestionCount, getQuestionsForTopics, slug,
} from './firestore.js';
...
export async function stepGenJob(jobId, ai = {}) {
  ...
  try {
    await runWithUsage(usage, async () => {
      ...
      const scoped = transcripts.length
        ? transcripts
