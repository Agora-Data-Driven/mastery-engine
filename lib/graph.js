...
import { programOf, DEFAULT_PROGRAM } from './programs.js';
...
// Hand-curated LESSON sequence for the courses whose lesson names carry no unit
// number of their own (everything else numbers them "01 …" and sorts naturally).
// KEEP IN SYNC with LESSON_ORDER in public/app.js.
//
// These lists are a FALLBACK now, not the rule: a lesson's stored `lessonOrder`
// outranks them. They still matter in two places — a course that has never been
// sequenced, and server.js's order backfill, which consults them before the
// alphabet so filling in a stored rank cannot overwrite what they say.
...
/** Min of `field` over `rows` — a group's rank is the lowest its rows carry, so a
 *  row that missed an order write can never drag its group backwards. */
const minRank = (rows, field) => rows.reduce(
  (m, r) => (Number.isFinite(r[field]) && r[field] < m ? r[field] : m), Infinity);

/** Order sibling groups (courses within a track, lessons within a course) by their
 *  stored rank, then by the hand-curated COURSE_ORDER, then natural name. Mirrors
 *  app.js's byCourseName/byLessonName so the map's spine matches the tree. */
function byGroupRank(groups, field, curated) {
  return (a, b) => {
    const oa = minRank(groups.get(a) || [], field), ob = minRank(groups.get(b) || [], field);
    if (oa !== ob) return oa - ob;
    const ra = curated?.has(a) ? curated.get(a) : Infinity;
    const rb = curated?.has(b) ? curated.get(b) : Infinity;
...
export function toNode(row, now = new Date()) {
  ...
  return {
    id: row.id,
    topic: row.topic || '',
    // program rides along so a map spanning programs (the shelf can) can tell
    // the clusters apart — same field the Progress tree carries for its scopes.
    program: row.program || DEFAULT_PROGRAM,
 ...
 * in study order — topics by `order`, lessons by `lessonOrder`, courses by
 * `courseOrder` (then the curated COURSE_ORDER, then name). Derived from the
 * catalog on demand — nothing to store, so it can never go stale when topics
 * are added or re-keyed.
 */
export function buildFlowEdges(catalog) {
  // (program, track) -> course -> lesson -> [rows]. Program is part of the key:
  // /api/graph passes a whole-bank catalog (the learner's shelf can span
  // programs), and same-named tracks in different programs must never chain
  // into each other. JSON key — no separator can collide with a track name.
  const tracks = new Map();
  for (const r of catalog) {
    if (!r.topic) continue;
    const key = JSON.stringify([programOf(r), r.track]);
    const t = tracks.get(key) || new Map();
  ...
  for (const courses of tracks.values()) {
    // Group rows by course / by lesson so a group's stored rank can be read off them.
    const courseRows = new Map([...courses].map(([c, ls]) => [c, [...ls.values()].flat()]));
    const courseNames = [...courses.keys()].sort(byGroupRank(courseRows, 'courseOrder', COURSE_RANK));
    let prevCourseTail = null;
    for (const cName of courseNames) {
      const lessons = courses.get(cName);
      const lessonNames = [...lessons.keys()].sort(byGroupRank(lessons, 'lessonOrder', null));
...
/**
 * Connectivity score (0..100) per node: how woven into the prerequisite fabric
 * a topic is. raw = weighted out-degree (what it unlocks) + weighted in-degree
 * (what it builds on), normalized against the best-connected node in the set.
 * Flow edges don't count — every topic sits on the curriculum spine, so they
 * carry no connectedness signal. Mutates and returns `nodes`.
 */
export function addConnectivity(nodes, prereqEdges) {
  const raw = new Map();
  for (const e of prereqEdges) {
    const w = Number.isFinite(e.w) ? e.w : PREREQ_WEIGHT_DEFAULT;
    raw.set(e.from, (raw.get(e.from) || 0) + w);
    raw.set(e.to, (raw.get(e.to) || 0) + w);
  }
  const max = Math.max(0, ...raw.values());
  for (const n of nodes) n.connectivity = max ? Math.round(((raw.get(n.id) || 0) / max) * 100) : 0;
  return nodes;
}
 ...
 *
 * `bank` (default: catalog) is where prerequisite STATS resolve. Pass the whole
 * bank when `catalog` is a launched-scope slice: prereq links cross programs,
 * so a prereq outside the slice would otherwise silently drop out. `catalog`
 * stays narrow on purpose — topic names are only unique within a program, and
 * the target must resolve to the launched program's row.
 */
export function prereqContext(topicName, catalog, links, { bank = catalog, now = new Date() } = {}) {
...
export function computeReadiness(targetIds, nodes, prereqEdges, { maxDepth = 3 } = {}) {
  ...
  if (!closure.length) {
    return { score: null, tier: 'unknown', ready: false, prereqCount: 0, strongCount: 0, weak: [], prereqs: [] };
  }
  ...
  for (const p of closure) {
    ...
    else if (untouched) { anyUntouched = true; if (p.w >= 3) criticalUntouched = true; }
  ...
  const weakRows = allRows.filter((r) => !r.strong);
  ...
  // Warm-up source: weak (topo-ordered) first, then strong (refresh) by priority —
  // so a learner can drill the prerequisites EVEN when already solid on them.
  const strongRows = allRows.filter((r) => r.strong).sort((a, b) => (b.priority ?? 0) - (a.priority ?? 0));
  const prereqRows = [...weakRows, ...strongRows];
