...
export const DEFAULT_MODEL = process.env.LMSTUDIO_MODEL || 'local-model';
...
export async function callLMStudio(prompt, { json = false, model } = {}) {
  ...
  let raw = '';
  let thought = 0;
  const u = (await readSse(res, (t, kind) => { if (kind === 'thinking') thought += t.length; else raw += t; })) || {};
  ...
  if (!text) {
    // Two very different failures look identical here, and guessing wrong sends you debugging the
    // wrong thing — so distinguish them by whether the model produced any reasoning at all.
    throw new Error(thought
      // It thought, then ran out of room to answer.
      ...
      // It emitted nothing whatsoever, which in practice means the prompt did not fit.
      : 'LM Studio returned an empty response — the prompt is most likely larger than the model\'s '
        + 'context window. Load a longer-context model, or lower LOCAL_GROUNDING_CHARS.');
  }
  return text;
}
