...
export async function callLMStudio(prompt, { json = false, model } = {}) {
  const post = (mode) => fetch(`${LMSTUDIO_HOST}/v1/chat/completions`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(chatBody(prompt, { model, json, mode })),
  });

  let res = await post(json ? (jsonMode || 'json_object') : 'off');
  if (!res.ok) {
    const txt = await res.text().catch(() => '');
    if (json && jsonMode !== 'off' && isResponseFormatComplaint(res.status, txt)) {
  ...
  if (json && jsonMode === null) jsonMode = 'json_object'; // it worked; remember and stop probing
  const data = await res.json();
  const raw = data?.choices?.[0]?.message?.content;
  if (!raw) throw new Error('LM Studio returned no content');
  const u = data?.usage || {};
  recordUsage({ provider: 'lmstudio', model: model || DEFAULT_MODEL, inputTokens: u.prompt_tokens || 0, outputTokens: u.completion_tokens || 0 });
  // A leaked <think> block would break JSON.parse downstream and read as gibberish in a hint.
...
export async function streamLMStudio(prompt, model, onToken) {
  ...
  let buf = '';
  let usage = null;
  // Route <think>…</think> to kind 'thinking' so callers that only want the answer can drop it,
  // and callers that surface a reasoning trace get one.
  ...
  for (;;) {
    ...
    while ((nl = buf.indexOf('\n')) >= 0) {
      ...
      try {
        ...
        // Some builds stream the scratchpad in its own field rather than inline tags.
        const think = obj?.choices?.[0]?.delta?.reasoning_content;
        if (think) onToken(think, 'thinking');
        if (t) emit(t);
