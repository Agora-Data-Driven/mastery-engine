...
/**
 * WHO a program's questions are written for.
 *
 * The Academy's three question generators used to open with the hardcoded line
 * "building an assessment for a working digital marketer" - true when the Academy
 * WAS the marketing bank, and quietly wrong ever since. Every AI-engineering,
 * data-engineering and RAG question was authored for a marketer, which is why so
 * many of them open "A digital marketer searches a content library...".
 *
 * The audience is a property of the PROGRAM, so it lives here rather than in a
 * prompt. A program doc may carry its own `audience` string (checked first by the
 * caller); this map covers the programs that exist, and the fallback is a neutral
 * phrase that biases the model toward nothing at all.
 */
const AUDIENCE = {
  data_science: 'a working data scientist',
  digital_marketing: 'a working digital marketer',
  ai_engineering: 'a working applied AI engineer',
  retrieval_augmented_generation_rag: 'an applied AI engineer building retrieval systems',
  agora_dev: 'a developer working on this team',
  agora_engineering: 'a software engineer running and extending these services',
  philosophy: 'a thoughtful general reader',
  spiritual: 'a thoughtful general reader',
};

/** The neutral fallback: it names no profession, so it skews no scenario. */
export const DEFAULT_AUDIENCE = 'a working practitioner of this subject';

/**
 * The practitioner a program's questions should be written for. `stored` is the
 * program doc's own `audience` field when it has one, so this can be overridden
 * per program in data without a deploy.
 */
export function audienceFor(programId, stored = '') {
  const own = String(stored || '').trim();
  if (own) return own;
  return AUDIENCE[String(programId || '').trim()] || DEFAULT_AUDIENCE;
}
