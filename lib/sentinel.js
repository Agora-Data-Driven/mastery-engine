...
/**
 * Sentinel's OWN self-knowledge doc — docs/HOW-SENTINEL-WORKS.md, served live by the deployed
 * Sentinel (`GET /api/internal/sentinel-guide`). The exact mirror of this app's HOW-IT-WORKS.md
 * pattern: one authoritative doc humans also read, injected into the assistant prompt, so the
 * assistant can answer "how do I use Sentinel for X?" from ground truth instead of guesses — and
 * because Sentinel serves it from its running code, every Sentinel DEPLOY updates the assistant's
 * knowledge with no step here.
 *
 * Cached in-process for 10 minutes (the doc changes on deploys, not per turn). NEVER throws: a
 * missing secret or unreachable Sentinel degrades to null and the assistant is simply less aware.
 */
let _guideCache = { text: null, at: 0 };
export async function sentinelGuide() {
  const secret = process.env.SSO_SECRET || '';
  const base = (process.env.SENTINEL_URL || 'https://sentinel.agoradatadriven.com').replace(/\/+$/, '');
  if (!secret) return null;
  const now = Date.now();
  if (_guideCache.text !== null && now - _guideCache.at < 10 * 60 * 1000) return _guideCache.text;
  try {
    const ts = String(Math.floor(now / 1000));
    const sig = createHmac('sha256', secret).update(`sentinel-guide:${ts}`).digest('hex');
    const r = await fetch(`${base}/api/internal/sentinel-guide`, {
      headers: { 'x-academy-ts': ts, 'x-academy-sig': sig },
      signal: AbortSignal.timeout(8000),
    });
    if (!r.ok) return _guideCache.text;   // keep serving the last good copy through a blip
    const data = await r.json();
    if (data && data.found && typeof data.guide === 'string') {
      _guideCache = { text: data.guide, at: now };
    }
    return _guideCache.text;
  } catch {
    return _guideCache.text;
  }
}
