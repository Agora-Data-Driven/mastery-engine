 ...
 *   gemini-flash  text input tokens + audio-output tokens (25 tokens/second of speech)
 *
 * Both are metered into the on-screen cost widget (`recordUsage({chars})` in lib/usage.js),
 * so voice spend shows up in the learner's own AI-spend panel.
 *
...
export const TTS_ENGINES = {
  'chirp3-hd': {
    label: 'Chirp 3 HD',
    note: 'Most natural, and quickest to start (~1s). Best for conversation. ~$30 per 1M characters.',
    // Chirp voices are selected purely by NAME; there is no model field for them.
    voiceName: (v) => `en-US-Chirp3-HD-${v}`,
    modelName: '',
    usageModel: 'chirp3-hd',
    stylable: false,
  },
  'gemini-flash': {
    label: 'Gemini Flash TTS',
    note: 'Steerable and token-priced, but ~3s slower to start speaking. Better for read-aloud than live talk.',
    // Gemini-TTS takes the bare voice name; the engine is carried by voice.model_name.
    voiceName: (v) => v,
    modelName: 'gemini-2.5-flash-tts',
    usageModel: 'gemini-2.5-flash-tts',
    stylable: true,
  },
};
...
export async function synthesize({ text, engine, voice, style } = {}) {
  ...
  const usageModel = cfg.usageModel || cfg.modelName || engine;
  recordUsage({ provider: 'tts', model: usageModel, chars: said.length, label: cfg.label });
  return {
