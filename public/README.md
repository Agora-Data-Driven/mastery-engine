...
| [academy-admin.html](academy-admin.html) / [academy-admin.js](academy-admin.js) | The admin "Composing Room" (2.8k lines JS). **Server-gated**: an explicit route in server.js 302s non-admins ahead of express.static. **Five stations** — Library · Compose · Questions · Chart · People — each carrying its modes behind an `.aa-seg` switcher rather than a rail entry (2026-09-07). Library is 01 and the default. | `wireLibrary` · `renderSourcePlan` · `commitSourcePlan` · `wireComposeModes` · `wireMergedStations` · `folderOf` / `renderFolderRail` · `pickedSource` · `isUnfiled` · panels `p-sources` / `p-curriculum` / `p-generate` / `p-roadmaps` / `p-people`; typed-SSE consumers for the planners |
...
9b. **Keep a panel from stacking** — the room's space rule: **one object stays visible, the
   ...
   AI* / *Build with AI* behind an `.aa-seg` switcher (`wireComposeModes`); Questions switches
