...
const App = (() => {
  ...
  function engineTag(o) {
    ...
    const patched = Array.isArray(o.patched) ? o.patched : [];
    // An authored artifact names its author instead of a model. Once a single
    // visual has been rewritten the page has TWO authors, so name both — a bare
    // "Made by Claude Code" would claim the whole page when part is now generated.
    if (o.locked) {
      return patched.length
        ? `✨ <b>Made by Claude Code</b> · visual${patched.length > 1 ? 's' : ''} ${esc(patched.join(', '))} rewritten${pretty ? ` by ${esc(pretty)}` : ''}`
        : '✨ <b>Made by Claude Code</b>';
    }
    if (!o.provider && !o.model) return '';
    const v = Number(o.attempt) > 1 ? ` · v${Number(o.attempt)}` : '';
    const p = patched.length ? ` · rewrote visual${patched.length > 1 ? 's' : ''} ${patched.join(', ')}` : '';
   ...
   * from inside the app.
   */
  const lockedConfirm = (subject, object) => `${subject} made by Claude Code, not generated by the app.\n\n`
    + `Regenerating replaces ${object} with a model version, and that cannot be undone from here.\n\nReplace anyway?`;
  ...
  async function syncGuideTools(pre) {
    ...
    if (tag && info.guide) {
      ...
      tag.innerHTML = engineTag(info.guide);
    }
  ...
  // Read-aloud: speak the assessment in whichever voice the learner picked (ttsSpeak, so the
  // cloud engines apply here too). Toggles off if it's already talking, which makes this button
  // its own replay — the second read is served from ttsCache and bills nothing. Strips
  // LaTeX/code markup so it reads naturally.
  ...
  // Hang a ↻ Replay control off a bubble we actually SPOKE. Mishearing a sentence is the normal
  // case in voice mode and re-asking for it is the expensive one: the answer gets regenerated by
  // the model AND re-synthesized, to say the same thing again. Replay says the same audio again
  // out of ttsCache instead — no model call, no TTS call, nothing billed.
  //
  // 🔴 Only on bubbles this session spoke. A bubble restored from history was never synthesized,
  // so a button on it would silently be a paid first read — the exact thing this exists to avoid.
  // (It is deliberately NOT added on the streaming/typed path for the same reason.)
  function addReplayButton(msgEl, text) {
    if (!msgEl) return;
    const bubble = msgEl.querySelector('.chat-bubble') || msgEl;
    if (bubble.querySelector('.chat-replay')) return;
    const btn = document.createElement('button');
    btn.type = 'button';
    btn.className = 'chat-replay';
    btn.textContent = '↻ Replay';
    btn.title = 'Hear this answer again — it re-uses the audio already made, so it costs nothing';
    btn.onclick = () => replaySpeech(text);
  ...
  async function sendAssistantBlocking(log, msg, spoken) {
    ...
    try {
      ...
      if (spoken) { speakAssistantReply(r.reply); addReplayButton(bubble, r.reply); }
      if (r.conversationId) {
  ...
  // ---- The replay cache ----------------------------------------------------------------
  // Every synthesized chunk is kept in memory keyed by engine|voice|text, so saying the SAME
  // words again is FREE — no /api/tts call, no characters billed, no audio tokens. That is what
  // makes ↻ Replay honest: mishearing a word must not cost what hearing it cost. Speaker Mode's
  // second 🔊 tap and a repeated voice preview land here too.
  //
  // Keyed on the engine and voice as well as the text, because the same sentence in Chirp and in
  // Gemini Flash are different audio — a key on text alone would replay the voice you just left.
  // Bounded by BYTES, not entries: one long answer is a dozen chunks and several MB, and a cache
  // that only counts entries would hold all of it forever. Map iteration order is insertion
  // order, and a hit re-inserts, so evicting from the front is least-recently-used.
  const TTS_CACHE_MAX_BYTES = 8 * 1024 * 1024;
  const ttsCache = new Map();
  let ttsCacheBytes = 0;

  const ttsCacheKey = (text) => `${ttsEngine()}|${ttsCloudVoice()}|${text}`;

  function ttsCacheGet(text) {
    const key = ttsCacheKey(text);
    const blob = ttsCache.get(key);
    if (!blob) return null;
    ttsCache.delete(key); ttsCache.set(key, blob);          // touch: move to the young end
    return blob;
  }

  function ttsCachePut(text, blob) {
    if (!blob || !blob.size || blob.size > TTS_CACHE_MAX_BYTES) return;
    const key = ttsCacheKey(text);
    const old = ttsCache.get(key);
    if (old) { ttsCache.delete(key); ttsCacheBytes -= old.size; }
    ttsCache.set(key, blob); ttsCacheBytes += blob.size;
    for (const [k, b] of ttsCache) {
      if (ttsCacheBytes <= TTS_CACHE_MAX_BYTES) break;
      ttsCache.delete(k); ttsCacheBytes -= b.size;
    }
  }

  // The cached front door to synthesis. A hit resolves immediately, which also means a replay
  // has no first-chunk wait at all — the 0.9s (Chirp) / 3.6s (Gemini) floor is only ever paid once.
  //
  // 🔴 Cache the BLOB, never an object URL: playTtsBlob revokes its URL after every play, and a
  // revoked URL replays as an instant error. The Blob itself survives that just fine.
  function speakChunk(text, seq) {
    const hit = ttsCacheGet(text);
    if (hit) return Promise.resolve(hit);
    return fetchTtsChunk(text, seq).then((blob) => { ttsCachePut(text, blob); return blob; });
  }
  ...
  async function cloudSpeak(text, seq, { onStart, onEnd } = {}) {
    ...
    let pending = speakChunk(chunks[0], seq).catch((e) => e);
    for (let i = 0; i < chunks.length; i++) {
      ...
      pending = i + 1 < chunks.length ? speakChunk(chunks[i + 1], seq).catch((e) => e) : null;
  ...
  // Say a reply AGAIN, from the top. Every chunk is already in ttsCache under this engine+voice,
  // so a replay is free and starts instantly — no first-chunk synthesis wait.
  //
  // 🔴 It goes back through speakAssistantReply while conversation mode is on, rather than
  // straight to ttsSpeak. That is not tidiness: the hands-free loop's echo guard compares what
  // the recognizer hears against convo.spoken, so audio played outside that dance is heard by
  // our own mic as the learner talking and fires a turn back at the assistant.
  function replaySpeech(text) {
    const said = plainForSpeech(text);
    if (!said) return;
    // Both paths funnel into ttsSpeak, which cancels first — so tapping Replay while it is still
    // talking restarts from the top rather than queueing a second voice. That is the ask: you tap
    // it because you missed the first few words.
    if (convoOn()) speakAssistantReply(text);
    else ttsSpeak(said);
  }
