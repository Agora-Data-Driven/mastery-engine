...
#     .\scripts\push-branch.ps1 -Desc quiz-fix        # -> alex/quiz-fix
#     .\scripts\push-branch.ps1 -Message "WIP quiz"   # custom commit message
#
...
$danger = $staged | Where-Object { $_ -match '(\.env$)|(\.env\.)|(\.p8$)|(\.pem$)|(\.key$)|(credentials.*\.json$)|(service-account.*\.json$)' }
if ($danger) {
